...
import top.kikt.imagescanner.ImageScanner.Companion.handler
...
class ImageScanner(val registrar: PluginRegistry.Registrar) {
    ...
    internal val gifPathDirIdMap = HashMap<String, ArrayList<Asset>>()
...
//        Log.i("K", "start scan")
    ...
    private fun handleImageCursor(mCursor: Cursor) {
        ...
        val mediaType = mCursor.getString(mCursor.getColumnIndex(MediaStore.Images.Media.MIME_TYPE))
        var type = AssetType.Image
        if (mediaType == "image/gif") {
            type = AssetType.Gif
        }
        val img = Asset(path, imgId, dir, dirId, title, thumb, type, date, null, width, height)
...
//            refreshThumb(r)
    ...
    fun getAllResource(call: MethodCall, result: MethodChannel.Result?) {
        threadPool.execute {
            scan()
            filterVideoPath()
            filterImagePath()
...
//        result.success(thumbHelper.getThumbData(img))
        when (img.type) {
            ...
            AssetType.Gif -> ThumbnailUtil.getThumbnailByGlide(registrar.activity(), img.path, width, height, result)
    ...
    private fun createImagePath(dirId: String, dir: String) {
        ...
        gifPathDirIdMap[dirId] = ArrayList()
    }
    ...
    private fun filterGifPath() {

        for (img in imgList) {
            if (img?.type == AssetType.Gif) {
                gifPathDirIdMap[img.dirId]?.add(img)
            }
        }
    }
...
fun ImageScanner.getAllGif(result: MethodChannel.Result) {
    val list = ArrayList<String>()
    for (entry in gifPathDirIdMap) {
        for (asset in entry.value) {
            list.add(asset.path)
        }
    }
