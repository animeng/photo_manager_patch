...
class PhotoManager {
  ...
  /// scan all resource in album
  static Future<bool> scanAlbum() async {
    var result = await _channel.invokeMethod("scanAlbum");
    return result;
  }
  ...
  ///
  /// 获取视频列表
  static Future<List<AssetEntity>> getAllVideoAsset() async {
    List<AssetEntity> list = await PhotoManager._getAssetList(AssetPathEntity._allVideo);
    return list;
  }
  ...
  /// 获取livephoto列表
  ///
  static Future<List<AssetEntity>> getAllLivePhotoAsset() async {
    List<AssetEntity> list = await PhotoManager._getAssetList(AssetPathEntity._livePhoto);
    return list;
  }
  ...
  /// 获取gif列表
  ///
  static Future<List<AssetEntity>> getAllGifAsset() async {
    List<AssetEntity> list = await PhotoManager._getAssetList(AssetPathEntity._gif);
    return list;
  }
  ...
  static Future<List<AssetEntity>> _getAssetList(AssetPathEntity path) async {
    ...
    if (path.onlyGif) {
      path.hasVideo = true;
      list = await _channel.invokeMethod("getAllGif");
      return _castAsset(list, AssetType.gif);
    } else if (path.onlyLivePhoto) {
      path.hasVideo = true;
      list = await _channel.invokeMethod("getAllLivePhoto");
      return _castAsset(list, AssetType.livePhoto);
    } else if (path.onlyVideo) {
      path.hasVideo = true;
      if (path.isAll) {
